...
#ifdef PCM_ALSA
#ifndef PCM_ALSA_DISABLE_HS
int app_hs_open_alsa_duplex(UINT16 handle);
...
void app_hs_cback(tBSA_HS_EVT event, tBSA_HS_MSG *p_data)
{
    ...
    case BSA_HS_REGISTER_EVT:
        ...
        {
            printf("ERROR : BSA_HS_REGISTER_EVT invalid handle=%d, status=%d\n", p_data->reg.hndl, p_data->reg.status);
            break;
        }
    ...
    case BSA_HS_BIND_EVT:
        fprintf(stdout, "BSA_HS_BIND_EVT:%s, handle %d\n", p_data->val.str, handle);
        break;
